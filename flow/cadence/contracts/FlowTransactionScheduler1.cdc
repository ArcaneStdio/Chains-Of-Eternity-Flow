...
access(all) contract FlowTransactionScheduler1 {
    ...
    access(all) resource interface TransactionHandler: ViewResolver.Resolver {
        ...
        access(Execute) fun executeTransaction(id: UInt64, data: AnyStruct?, withdrawref: auth(NonFungibleToken.Withdraw) &{NonFungibleToken.Collection})
    }
    ...
    access(all) resource ScheduledTransaction {
        ...
        access(all) view fun status(): Status? {
            return FlowTransactionScheduler1.sharedScheduler.borrow()!.getStatus(id: self.id)
        }
    ...
    access(all) struct TransactionData {
        ...
        access(contract) let withdrawRef: auth(NonFungibleToken.Withdraw) &{NonFungibleToken.Collection}
        ...
        access(contract) fun payAndRefundFees(refundMultiplier: UFix64): @FlowToken.Vault {
            ...
            if refundMultiplier == 0.0 {
                FlowFees.deposit(from: <-FlowTransactionScheduler1.withdrawFees(amount: self.fees))
                return <-FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>())
            } else {
                let amountToReturn = self.fees * refundMultiplier
                let amountToKeep = self.fees - amountToReturn
                let feesToReturn <- FlowTransactionScheduler1.withdrawFees(amount: amountToReturn)
                FlowFees.deposit(from: <-FlowTransactionScheduler1.withdrawFees(amount: amountToKeep))
                return <-feesToReturn
            }
        }
        ...
        access(contract) view fun getWithdrawRef(): auth(NonFungibleToken.Withdraw) &{NonFungibleToken.Collection} {
            return self.withdrawRef
        }
    }
    ...
    access(all) resource SharedScheduler {
        ...
        access(contract) fun schedule(
            ...
            data: AnyStruct?,
            witdhdrawref: auth(NonFungibleToken.Withdraw) &{NonFungibleToken.Collection},
            timestamp: UFix64,
            priority: Priority,
            executionEffort: UInt64,
            ...
            FlowTransactionScheduler1.depositFees(from: <-fees)
        ...
        access(contract) fun estimate(
            ...
            let dataSizeMB = FlowTransactionScheduler1.getSizeOfData(data)
        ...
        access(Execute) fun executeTransaction(id: UInt64) {
            ...
            transactionHandler.executeTransaction(id: id, data: tx.getData(), withdrawref: tx.getWithdrawRef())
        }
    }
    ...
    access(all) init() {
        self.storagePath = /storage/sharedScheduler1
