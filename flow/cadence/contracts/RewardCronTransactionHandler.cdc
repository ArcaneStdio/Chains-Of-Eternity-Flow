...
access(all) contract RewardCronTransactionHandler {

    /// Struct to hold cron configuration data (immutable for transaction serialization)
    access(all) struct RewardCronConfig {
        ...
        access(all) fun withIncrementedCount(): RewardCronConfig {
            return RewardCronConfig(
    ...
    access(all) resource Handler: FlowTransactionScheduler.TransactionHandler {
        access(FlowTransactionScheduler.Execute) fun executeTransaction(id: UInt64, data: AnyStruct?) {
            ...
            let cronConfig = data as! RewardCronConfig? ?? panic("RewardCronConfig data is required")
            let updatedConfig = cronConfig.withIncrementedCount()

            // Check if we should continue scheduling
            if !updatedConfig.shouldContinue() {
                log("Reward cron job completed after ".concat(updatedConfig.executionCount.toString()).concat(" executions"))
                return
            }
            ...
            if RewardCronTransactionHandler.account.storage.borrow<&AnyResource>(from: /storage/RewardCronTransactionHandler) == nil {
                let handler <- RewardCronTransactionHandler.createHandler()
                RewardCronTransactionHandler.account.storage.save(<-handler, to: /storage/RewardCronTransactionHandler)
            }

            // Withdraw FLOW fees from this contract's account vault
            let vaultRef = RewardCronTransactionHandler.account.storage
            ...
            let handlerCap = RewardCronTransactionHandler.account.capabilities.storage
            ...
            log("Next Reward cron transaction scheduled (id: ".concat(receipt.id.toString()).concat(") at ").concat(receipt.timestamp.toString()).concat(" (execution #").concat(updatedConfig.executionCount.toString()).concat(")"))
    ...
    access(all) fun createRewardCronConfig(intervalSeconds: UFix64, baseTimestamp: UFix64?, maxExecutions: UInt64?): RewardCronConfig {
        let base = baseTimestamp ?? getCurrentBlock().timestamp
        return RewardCronConfig(intervalSeconds: intervalSeconds, baseTimestamp: base, maxExecutions: maxExecutions, executionCount: 0)
    }
}
